...
export function Collection({title,children,actions,loading=false,error='',onRetry}:{title:string;children:ReactNode;actions?:ReactNode;loading?:boolean;error?:string;onRetry?:()=>void}) {
  ...
  const filtered=rows.filter(row=>searchable(row).replace(/\s+/g,' ').toLowerCase().includes(query.trim().replace(/\s+/g,' ').toLowerCase()));
  ...
  return <section className="card collection" aria-busy={loading}>
    ...
    {error?<div className="collection-empty" role="alert"><h3>Unable to load {title.toLowerCase()}</h3><p>{error}</p>{onRetry&&<button className="secondary" onClick={onRetry}>Try again</button>}</div>:loading?<LoadingPanel label={`Loading ${title.toLowerCase()}…`}/>:filtered.length===0?<div className="collection-empty"><h3>{query?'No matches found':`No ${title.toLowerCase()} yet`}</h3><p>{query?'Try a different search or clear your search.':title==='Progress history'?'Start with one measurement. Each entry adds perspective to your journey.':title==='Client access'?'Your coaching practice starts with a person. Add your first client above.':title==='Check-in submissions'?'Client reflections will appear here when they’re ready for your review.':'New records will appear here.'}</p></div>:<div className="collection-rows">{filtered.slice((current-1)*size,current*size)}</div>}
