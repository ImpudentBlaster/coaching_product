...
async function audit(client, actor, action, id, data) {
...
async function log(client, actor, action, id, revision, snapshot) {
...
async function pair(client, coachId, clientId) {
    ...
    if (!result.rowCount)
        throw new CheckinError('Approved client relationship not found', 404);
}
async function assignment(client, id, actor, coach) {
    const result = await client.query(`SELECT * FROM checkin_assignments WHERE id=$1 AND ${coach ? 'coach_id' : 'client_id'}=$2 FOR UPDATE`, [id, actor]);
    if (!result.rows[0])
        throw new CheckinError('Check-in not found', 404);
    await pair(client, result.rows[0].coach_id, result.rows[0].client_id);
    return result.rows[0];
}
async function reserveDate(client, coachId, clientId, formId, date, exceptId) {
    ...
    if (existing.rowCount)
        throw new CheckinError(`This form is already assigned to this client on ${date}. Choose another date or form.`);
}
...
function errors(error, _request, response, next) {
    if (error && typeof error === 'object' && 'type' in error && error.type === 'entity.too.large') {
        response.status(413).json({ message: 'Choose a photo smaller than 8 MB.' });
        return;
    }
    if (error instanceof z.ZodError) {
        response.status(400).json({ message: 'Check the submitted fields', details: error.flatten() });
        return;
    }
...
function photoRoutes(router, pool, coach) {
    ...
    router.get('/:id/photos/:photoId', async (request, response) => {
        ...
        if (!bytes)
            throw new CheckinError('Photo not found', 404);
    ...
    router.post('/:id/photos', rateLimit({ windowMs: 60000, limit: 20, keyGenerator: request => request.auth.userId, standardHeaders: true, legacyHeaders: false, message: { message: 'Too many photo uploads. Please wait a minute.' } }), raw({ type: 'application/octet-stream', limit: '8mb', inflate: false }), async (request, response) => {
        ...
        if (!Buffer.isBuffer(body))
            throw new CheckinError('Upload the photo as binary image data', 400);
        const photo = await tx(pool, async (client) => {
            ...
            if (submitted.rowCount)
                throw new CheckinError('Photos are locked after submission');
            if ((await photoStorage.list(client, id)).length >= 5)
                throw new CheckinError('A check-in can contain up to 5 photos', 400);
            let normalized;
            try {
                normalized = await normalizeCheckinPhoto(body);
            }
            catch (error) {
                throw new CheckinError(error instanceof Error ? error.message : 'Invalid photo', 400);
            }
    ...
    router.delete('/:id/photos/:photoId', async (request, response) => {
        ...
        await tx(pool, async (client) => {
            ...
            if (submitted.rowCount)
                throw new CheckinError('Photos are locked after submission');
            if (!await photoStorage.remove(client, id, photoId))
                throw new CheckinError('Photo not found', 404);
...
export function createCoachCheckins(pool) {
    const router = Router();
    photoRoutes(router, pool, true);
    router.get('/forms', async (request, response) => {
        const result = await pool.query('SELECT id,version,definition,is_default "isDefault" FROM checkin_forms WHERE coach_id=$1 ORDER BY updated_at DESC', [request.auth.userId]);
    ...
    async function saveForm(request, response, id) {
        ...
        const form = await tx(pool, async (client) => {
            ...
            if (id) {
                ...
                const existing = await client.query('SELECT id,version FROM checkin_forms WHERE id=$1 AND coach_id=$2 FOR UPDATE', [id, request.auth.userId]);
                if (!existing.rows[0])
                    throw new CheckinError('Form not found', 404);
                if (existing.rows[0].version !== input.version)
                    throw new CheckinError('This form changed. Reload before saving.');
                version = existing.rows[0].version + 1;
            }
            if (input.isDefault)
                await client.query('UPDATE checkin_forms SET is_default=false WHERE coach_id=$1 AND is_default', [request.auth.userId]);
            const saved = id ? await client.query('UPDATE checkin_forms SET definition=$1,version=$2,is_default=$3,updated_at=now() WHERE id=$4 RETURNING id,version,definition,is_default "isDefault"', [JSON.stringify(input.definition), version, input.isDefault, id]) : await client.query('INSERT INTO checkin_forms(coach_id,definition,is_default) VALUES($1,$2,$3) RETURNING id,version,definition,is_default "isDefault"', [request.auth.userId, JSON.stringify(input.definition), input.isDefault]);
    ...
    router.post('/', async (request, response) => {
        ...
        const result = await tx(pool, async (client) => {
            const form = await client.query(`SELECT id,version,definition FROM checkin_forms WHERE coach_id=$1 AND ${input.formId ? 'id=$2' : 'is_default=true'} FOR SHARE`, input.formId ? [request.auth.userId, input.formId] : [request.auth.userId]);
            if (!form.rows[0])
                throw new CheckinError('Choose a published form', 404);
    ...
    router.post('/:id/reschedule', async (request, response) => {
        ...
        await tx(pool, async (client) => {
            ...
            const submitted = await client.query("SELECT id FROM checkin_submissions WHERE assignment_id=$1 AND status<>'DRAFT'", [id]);
            if (submitted.rowCount)
                throw new CheckinError('Submitted check-ins cannot be rescheduled');
            ...
            if (!updated.rowCount)
                throw new CheckinError('The due date changed. Reload before rescheduling.');
    ...
    router.post('/:id/review', async (request, response) => {
        ...
        await tx(pool, async (client) => {
            ...
            const saved = await client.query("UPDATE checkin_submissions SET status='REVIEWED',review_status=$1,review_notes=$2,reviewed_at=now(),reviewed_by=$3,revision=revision+1,updated_at=now() WHERE assignment_id=$4 AND status IN ('SUBMITTED','REVIEWED') AND revision=$5 RETURNING *", [input.status, input.notes, request.auth.userId, id, input.revision]);
            if (!saved.rows[0])
                throw new CheckinError('Check-in changed or has not been submitted. Reload before reviewing.');
    ...
    router.get('/:id/history', async (request, response) => {
        ...
        const history = await tx(pool, async (client) => {
            await assignment(client, id, request.auth.userId, true);
            return (await client.query('SELECT id,actor_id "actorId",action,revision,snapshot,created_at "createdAt" FROM checkin_logs WHERE assignment_id=$1 ORDER BY created_at,id', [id])).rows;
        });
...
export function createClientCheckins(pool) {
    const router = Router();
    ...
    async function save(request, response, submit) {
        const id = z.string().uuid().parse(request.params.id);
        ...
        const result = await tx(pool, async (client) => {
            ...
            const existing = (await client.query('SELECT * FROM checkin_submissions WHERE assignment_id=$1 FOR UPDATE', [id])).rows[0];
            if (existing && existing.status !== 'DRAFT')
                throw new CheckinError('Submitted check-ins are locked');
            if ((existing?.revision ?? 0) !== input.revision)
                throw new CheckinError('This draft changed. Reload before saving.');
            let answers;
            try {
                answers = validateAnswers(row.form_snapshot, input.answers, submit);
            }
            catch (error) {
                throw new CheckinError(error instanceof Error ? error.message : 'Invalid answers', 400);
            }
            const flags = submit ? flagsFor(row.form_snapshot, answers) : [];
            const photos = await photoStorage.list(client, id);
            if (submit && photos.length === 0)
                throw new CheckinError('Upload at least one photo before submitting your check-in', 400);
            const saved = await client.query(`INSERT INTO checkin_submissions(assignment_id,coach_id,client_id,data,status,submitted_snapshot,submitted_at,flags) VALUES($1,$2,$3,$4,$5::record_status,$6,CASE WHEN $5='SUBMITTED' THEN now() ELSE NULL END,$7) ON CONFLICT(assignment_id) DO UPDATE SET data=excluded.data,status=excluded.status,submitted_snapshot=excluded.submitted_snapshot,submitted_at=excluded.submitted_at,flags=excluded.flags,revision=checkin_submissions.revision+1,updated_at=now() RETURNING *`, [id, row.coach_id, row.client_id, JSON.stringify(answers), submit ? 'SUBMITTED' : 'DRAFT', submit ? JSON.stringify(answers) : null, JSON.stringify(flags)]);
            if (submit)
                await client.query('UPDATE checkin_submissions SET submitted_photo_ids=$1 WHERE assignment_id=$2', [photos.map(photo => photo.id), id]);
