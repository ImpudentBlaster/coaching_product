...
export function sumNutrients(values) {
...
export function portion(nutrients, quantity, servingSize) {
...
async function audit(client, coach, action, id, metadata) {
...
export function createNutritionCoachRouter(pool) {
    const router = Router();
    router.get('/', async (request, response) => {
        const result = await pool.query('SELECT id,data,version,created_at "createdAt" FROM nutrition_library WHERE coach_id=$1 AND archived_at IS NULL ORDER BY updated_at DESC,id', [request.auth.userId]);
        const assignments = await pool.query('SELECT id,client_id "clientId",plan_id "planId",assigned_at "assignedAt" FROM nutrition_plan_assignments WHERE coach_id=$1 AND active=true', [request.auth.userId]);
        response.json({ entries: result.rows, assignments: assignments.rows });
    });
    async function saveEntry(request, response) {
        ...
        if (!kind.success || !parsed.success)
            return response.status(400).json({ message: 'Check the nutrition fields, quantities and selections.' });
        const input = parsed.data;
        const id = request.params.id;
        const version = z.object({ version: z.number().int().positive() }).safeParse(body);
        if (id && (!z.string().uuid().safeParse(id).success || !version.success))
            return response.status(400).json({ message: 'Reload the entry before editing.' });
        try {
            const entry = await transaction(pool, async (client) => {
                const node = { kind: input.kind, name: input.name, notes: input.notes };
                if (input.kind === 'foods')
                    Object.assign(node, { servingSize: input.servingSize, unit: input.unit, nutrients: input.nutrients });
                else {
                    const childKind = input.kind === 'meals' ? 'foods' : input.kind === 'days' ? 'meals' : 'days';
                    const children = await client.query('SELECT id,data FROM nutrition_library WHERE coach_id=$1 AND kind=$2 AND archived_at IS NULL AND id=ANY($3::uuid[])', [request.auth.userId, childKind, input.items.map(item => item.id)]);
                    node.items = input.items.map(item => {
                        const child = children.rows.find(row => row.id === item.id)?.data;
                        if (!child)
                            throw new NutritionError('A selected entry is unavailable. Choose entries from your own library.');
                        if ('quantity' in item)
                            return { id: item.id, quantity: item.quantity, node: child, nutrients: portion(child.nutrients, item.quantity, child.servingSize) };
                        return { id: item.id, label: item.label, node: child, ...(child.nutrients ? { nutrients: child.nutrients } : {}) };
                    });
                    if (input.kind !== 'plans')
                        node.nutrients = sumNutrients(node.items.map(item => item.nutrients));
                }
                const serialized = JSON.stringify(node);
                if (Buffer.byteLength(serialized) > 512000)
                    throw new NutritionError('This plan is too large. Use fewer days or meals.');
                const result = id ? await client.query('UPDATE nutrition_library SET data=$1,version=version+1,updated_at=now() WHERE id=$2 AND coach_id=$3 AND kind=$4 AND version=$5 AND archived_at IS NULL RETURNING id,data,version,created_at "createdAt"', [serialized, id, request.auth.userId, input.kind, version.success ? version.data.version : 0]) : await client.query('INSERT INTO nutrition_library(coach_id,kind,data) VALUES($1,$2,$3) RETURNING id,data,version,created_at "createdAt"', [request.auth.userId, input.kind, serialized]);
                if (!result.rows[0])
                    throw new NutritionError('Entry changed or is unavailable. Reload the list and try again.');
    ...
    router.delete('/:kind/:id', async (request, response) => {
        if (!z.string().uuid().safeParse(request.params.id).success || !kinds.safeParse(request.params.kind).success)
            return response.status(400).json({ message: 'Invalid entry' });
        const removed = await transaction(pool, async (client) => {
            const row = await client.query('UPDATE nutrition_library SET archived_at=now(),updated_at=now() WHERE id=$1 AND coach_id=$2 AND kind=$3 AND archived_at IS NULL RETURNING id', [request.params.id, request.auth.userId, request.params.kind]);
            if (row.rows[0])
                await audit(client, request.auth.userId, 'NUTRITION_DELETED', row.rows[0].id, {});
    ...
    router.post('/plans/:id/assign', async (request, response) => {
        ...
        if (!parsed.success || !z.string().uuid().safeParse(request.params.id).success)
            return response.status(400).json({ message: 'Choose a valid client and plan.' });
        try {
            const assignment = await transaction(pool, async (client) => {
                ...
                if (!user.rowCount || !relationship.rowCount)
                    throw new NutritionError('Approved client not found.');
                const plan = await client.query('SELECT id,data FROM nutrition_library WHERE id=$1 AND coach_id=$2 AND archived_at IS NULL AND kind=\'plans\'', [request.params.id, request.auth.userId]);
                if (!plan.rows[0])
                    throw new NutritionError('Plan not found.');
                ...
                const result = await client.query('INSERT INTO nutrition_plan_assignments(coach_id,client_id,plan_id,snapshot) VALUES($1,$2,$3,$4) RETURNING id', [request.auth.userId, parsed.data.clientId, plan.rows[0].id, JSON.stringify(plan.rows[0].data)]);
...
export function createNutritionClientRouter(pool) {
    const router = Router();
    router.get('/', async (request, response) => {
        const result = await pool.query(`SELECT a.id,a.snapshot,a.assigned_at "assignedAt" FROM nutrition_plan_assignments a JOIN coach_clients cc ON cc.coach_id=a.coach_id AND cc.client_id=a.client_id JOIN users coach ON coach.id=a.coach_id WHERE a.client_id=$1 AND a.active=true AND cc.status='APPROVED' AND coach.account_status='APPROVED'`, [request.auth.userId]);
