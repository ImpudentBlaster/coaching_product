...
const repositoryRoot=resolve(process.cwd(),'../..');const sourcePath=resolve(repositoryRoot,'exercises_1.json');const imageDirectory=resolve(repositoryRoot,'exercise_images');
...
try{const client=await pool.connect();try{await client.query('BEGIN');for(const exercise of exercises){const gifAvailable=validImages.has(`${exercise.id}.gif`);await client.query(`INSERT INTO exercises(external_id,name,body_part,equipment,target,secondary_muscles,instructions,gif_available) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(external_id) DO UPDATE SET name=excluded.name,body_part=excluded.body_part,equipment=excluded.equipment,target=excluded.target,secondary_muscles=excluded.secondary_muscles,instructions=excluded.instructions,gif_available=excluded.gif_available,updated_at=now()`,[exercise.id,exercise.name,exercise.bodyPart,exercise.equipment,exercise.target,JSON.stringify(exercise.secondaryMuscles),JSON.stringify(exercise.instructions),gifAvailable]);imported++;}await client.query('COMMIT');}catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}}finally{await pool.end();}
